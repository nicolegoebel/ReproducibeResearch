...
```
The mean number of steps taken was: `r meanSteps`  

The median number of steps taken was: `r medianSteps`

A histogram of the total number of steps taken each day is shown below:
```{r figure total number of steps, fig.height=4}
...
```{r figure daily activity pattern, fig.height=4}
datesVec<-dat$date
...
On average across all the days in the dataset, the `r maxInt`th five-minute interval contains the maximum number of steps (`r maxSteps`).
...
```
The total number of missing values in the dataset are `r numNAs`.
...
Using the new data set with substituted NA values, the mean (`r newMean`) and median (`r newMedian`) are different than calculated when ignoring NA values.

The total number of steps with the new data set (`r totalNewSteps`) is `r diffSteps` steps greater than that calculated when ignoring NA values. Using an average value for all average step intervals is likely to overestimate the average steps.
